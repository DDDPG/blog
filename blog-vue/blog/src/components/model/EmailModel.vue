<template>
  <v-dialog v-model="emailFlag" max-width="460">
    <v-card class="login-container" style="border-radius: 4px">
      <v-icon class="float-right colorFlag" @click="emailFlag = false"> mdi-close </v-icon>
      <div class="login-wrapper">
        ...
        <!-- 按钮 -->
        <v-btn class="mt-7 " block style="background: #4caf50; color: #fff" @click="saveUserEmail">
          绑定
        </v-btn>
      </div>
    </v-card>
...
<script>
import { getCode, updateEmail } from "@/api/user";
export default {
  name: "EmailModel",
  data() {
    return {
