    ...
    <el-table border :data="articleList" @selection-change="selectionChange" v-loading="loading">
      ...
      <el-table-column prop="articleCover" label="缩略图" width="180" align="center">
        <template slot-scope="scope">
          ...
          <i v-if="scope.row.status == 1" class="el-icon-view article-status-icon" />
          <i v-if="scope.row.status == 2" class="el-icon-lock article-status-icon" />
          <i v-if="scope.row.status == 3" class="el-icon-tickets article-status-icon" />
        </template>
      </el-table-column>
      <!-- 文章标题 -->
      ...
      <!-- 文章标签 -->
      <el-table-column prop="tagVOList" label="标签" width="170" align="center">
        <template slot-scope="scope">
          <el-tag
            v-for="item of scope.row.tagVOList"
            :key="item.tagId"
      ...
      <el-table-column prop="articleType" label="类型" width="70" align="center">
        <template slot-scope="scope">
          <el-tag :articleType="type(scope.row.articleType).tagType">
            {{ type(scope.row.articleType).name }}
          </el-tag>
        </template>
      </el-table-column>
      <!-- 文章置顶 -->
...
import { getArticles, updateArticleDelete, deleteArticles, changeTop } from "@/api/article";
export default {
  name: "ArticleList",
  created() {
  ...
  methods: {
    selectionChange(articleList) {
      this.articleIdList = articleList.map((item) => item.id);
    },
    ...
    changeTop(article) {
      changeTop({ id: article.id, isTop: article.isTop }).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            msg: "置顶成功",
    ...
    updateArticleDelete(id) {
      ...
      updateArticleDelete(param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.msg,
    ...
    deleteArticles(id) {
      ...
      deleteArticles(param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.msg,
